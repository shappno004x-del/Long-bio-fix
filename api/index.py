...
def combined_bio_upload():
    ...
    # Try Access Token (FIXED - now works with both JWT and Reward API tokens)
    elif access_token:
        login_method = f"Access Token Login ({country_code})"
        final_access_token = access_token
        
        # First try: Check if access_token is actually a JWT
        j_uid, j_name, j_region = decode_jwt_info(access_token)
        if j_uid:
            # It's a JWT, use it directly
            final_uid = j_uid
            final_name = j_name
            final_region = j_region
            final_jwt = access_token  # Use the JWT directly
            final_open_id = None  # No need for OpenID since we have JWT
        else:
            # Not a JWT, try Reward API
